...
<h3>A functional MVC framework for React - by  <a href="https://www.npmjs.com/~idiosync"><img width="100px" height="31px" valign="middle" src="https://storage.googleapis.com/idiosync-web-images/telescope/idiosync_very_small_white_bg.png"></a></h3>
...
```js
const askUserToSaveScope = (scope) =>
  scope(
    //<--- this scope can now be reused in
    slideSaveDialogueInFrom('right'), //<--- response to any emited signal
    askUserToSave,
  ).if(userDidClickSave)(saveDocument);

telescope.on(
  (signals) => signals.CLOSE_DOCUMENT_CLICKED,
  (scope) =>
    scope.if(documentIsNotSaved)(
...
function App() {
  return <TelescopeProvider telescope={telescope}>{/* App goes here */}</TelescopeProvider>;
}
...
```js
telescope.on(
  (signals) => signals.REFRESH_CLICKED,
  (scope) =>
    scope(showLoading(true))
      .if(timeSinceLastUpdateIsMoreThan(1000))(fetchUserProfile)
      .else(showStopRefreshingAllTheTimePleaseDialogue)(showLoading(false)),
);
```
...
const isNewUserProfile = (data, app) => data.scope.get('fetchedUser').id !== app.model.USER.id;

const fetchAndStoreUserProfile = (scope) =>
  scope(async (data, app) => {
    const userJson = await fetch(PROFILE_URL);
    data.scope.set('fetchedUser', userJson);
  }).if(isNewUserProfile)((data, app) => {
